cc.Class({
    ...
    setHeader(){//设置头部
        if(this.getPerNode()){
            this.headerInfo[0].string = this.perNode.getComponent('PersistNode').userData.fHeaderInfo.nickname;
            this.headerInfo[1].string = this.perNode.getComponent('PersistNode').userData.fHeaderInfo.level;
            this.headerInfo[2].string = this.perNode.getComponent('PersistNode').userData.fHeaderInfo.jewel;
            this.headerInfo[3].string = this.perNode.getComponent('PersistNode').userData.fHeaderInfo.gold;
            this.userPic.spriteFrame = this.spriteList[this.perNode.getComponent('PersistNode').userData.fHeaderInfo.pic-1||0];
        };
    },
    ...
    renderTree(tree,type,status,id){//渲染单个林场——类型-状态
        ...
        tree.on(cc.Node.EventType.TOUCH_END,function(){
            ...
            //场景跳转
            cc.director.loadScene("FriendPlantDetail",function(){//回调
